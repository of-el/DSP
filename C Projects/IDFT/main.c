...
void calc_dft(double* sig_src_arr, double* sig_dest_rex_arr, double* sig_dest_imx_arr, int sig_length);
...
int main()
{
    ...
    calc_dft((double*) &InputSignal_f32_1kHz_15kHz[0], (double*) &Output_REX[0], (double*) &Output_IMX[0], (int) SIG_LENGTH);
...
void calc_idft(double* idft_out_arr, double* sig_src_rex_arr, double* sig_src_imx_arr, int idft_length)
{
    ...
    for(k = 0; k < idft_length/2; k++)
    {
        for(j = 0; j < idft_length; j++)
        {
            idft_out_arr[j] = idft_out_arr[j] + sig_src_rex_arr[k] * cos(2*PI*k*j/idft_length);
            idft_out_arr[j] = idft_out_arr[j] + sig_src_imx_arr[k] * sin(2*PI*k*j/idft_length);
        }
    }
}
